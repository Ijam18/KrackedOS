...
### **BMAD Method System Updates**
**Triggers**:
- `bmad help` reveals a stable route conclusion or active phase
- `bmad plan` produces a reusable work sequence
- `bmad review` surfaces meaningful risks or recurring quality issues
- `bmad edge-cases` reveals hidden failure patterns worth remembering
- `bmad distill` creates a reusable context brief

**Auto-Update Process**:
```markdown
1. IDENTIFY: Decide whether the BMAD output is temporary or durable
2. ROUTE: Map the durable conclusion to the correct MAJI memory file
3. SUMMARIZE: Save the insight, not the raw workflow noise
4. APPLY: Use the BMAD result immediately in current work
5. VERIFY: Confirm the saved memory will help future Kracked_OS sessions
```

**Routing Rules**:
- **current-session.md**: Current phase, route conclusion, active plan summary, immediate next step
...
- **Routes** durable BMAD outputs into MAJI memory instead of leaving them as temporary workflow chatter
...
- **Grounded**: Must respect current repo/code truth over stale memory
...
6. **Truth Precedence**: Does current repo/code truth override stale memory where needed?
...
- [ ] If BMAD is active, route durable workflow outcomes into the correct MAJI files
...
- [ ] Save BMAD route conclusions, review findings, and plan summaries to the right memory locations
