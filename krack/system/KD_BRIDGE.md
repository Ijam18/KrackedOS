...
This document explains how `tools/Kracked_Skills_Agent-main/` relates to `krack/`.

## Purpose

`krack/` is the canonical internal memory-and-method layer for KRACKED_OS.
...
The following overlap is now treated as krack-owned doctrine:
...
| KD concept | krack equivalent |
|---|---|
| KD system prompt | `krack/system/` doctrine |
| KD skills | split by ownership: repo-owned reusable doctrine belongs in `krack/skills/local-skills/`, KD project-local installed skills stay under `.kracked/skills/`, and tool execution surfaces may appear in places like `.agents/skills/` |
| KD workflows | `krack/method/` and BMAD routing |
| KD memory patterns | `krack/engine/memory/` + MAJI memory files |
| KD adapters | external execution surfaces such as `.agents/skills/` |
| KD runtime events / observer | KD-only external runtime |

## Usage Rule

Use krack-native flows first when the task is about:
...
If KD-related work becomes repeated enough inside this repo, create a dedicated krack local skill for KD integration.

Do not merge KD runtime code into krack unless KRACKED_OS later needs a real embedded KD runtime.
