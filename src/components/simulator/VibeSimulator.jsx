...
const FlowCanvas = ({ compact = false, quickAddType = null, onQuickAddConsumed = null }) => {
    ...
    useEffect(() => {
        ...
        const newNode = {
            ...
            type: quickAddType,
            position,
            data: { label: quickAddType.toUpperCase() },
        };
        setNodes((nds) => nds.concat(newNode));
        onQuickAddConsumed?.();
...
export default function VibeSimulator() {
        ...
        <div style={{ width: '100%', height: '100%', display: 'flex', position: 'relative', background: 'linear-gradient(180deg, #f7faff 0%, #edf3fb 100%)' }}>
            ...
            {isCompact && panelOpen && (
                <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, zIndex: 25 }}>
                    <SimulatorSidebar compact onClose={() => setPanelOpen(false)} onQuickAdd={setQuickAddType} />
                </div>
            )}
            {isCompact && !panelOpen && (
                <button
